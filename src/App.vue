...
export default {
  ...
  created() {
    ...
    if (horarioGuardado) {
      this.clases = JSON.parse(horarioGuardado);
    }
  },
  methods: {
    ...
    verificarChoque(nuevoGrupo) {
      for (const bloqueNuevo of nuevoGrupo.horarios) {
        ...
        for (const claseExistente of this.clases) {
    ...
    agregarGrupoAHorario(grupo) {
      if (this.verificarChoque(grupo)) {
        this.buscarGifDeChoque();
        return;
      }

      const nombresDeCursosUnicos = this.clases.map(c => c.nombre).filter((v, i, a) => a.indexOf(v) === i);
      const colorDelGrupo = this.coloresDisponibles[nombresDeCursosUnicos.length % this.coloresDisponibles.length];

      grupo.horarios.forEach(horario => {
        const nuevaClase = {
          id: `${grupo.siglaCurso}-${grupo.numero}-${horario.dia}-${horario.horaInicio}`,
          nombre: `${grupo.nombreCurso} (G${grupo.numero})`,
          dia: horario.dia,
        ...
        if (!this.clases.some(c => c.id === nuevaClase.id)) {
